...
namespace ProP_Status
{
    class Food
    {
        ...
        public int QuantityAvailable { get { return this.quantityAvailable; } }
        ...
        public static void CheckIfQuantityIsNotCritical(List<Food> foodList)
        {
            foreach(Food f in foodList)
            {
                if(f.quantityAvailable <= 1000)
                {
                    MessageBox.Show(f.Name + " quantity is in a critical state. Please take action.");
                }
            }
        }
